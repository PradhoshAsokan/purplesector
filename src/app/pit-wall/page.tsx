...
export default function PitWallPage() {
  ...
  useEffect(() => {
    ...
    async function fetchLive(apiUrl: string) {
      ...
      // 4. ADVANCED LEADERBOARD MERGE (Position + Intervals)
      Promise.all([
      ...
      fetch(`${apiUrl}/test/radio`).then(r => r.json()).then(data => {
        if (data.status === 'Success' && Array.isArray(data.data)) setRadio(data.data.slice(-8).reverse());
      });
      ...
      fetch(`${apiUrl}/test/pits`).then(r => r.json()).then(data => {
        if (data.status === 'Success' && Array.isArray(data.data)) setPits(data.data.slice(-5).reverse());
      });
    }
      ...
      <div className="h-[calc(100vh-120px)] flex flex-col mb-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 flex-1 min-h-0">
          ...
          <div className="lg:col-span-6 h-full border border-[#1F1F1F] bg-[#1F1F1F]/10 rounded-lg flex flex-col items-center justify-center relative overflow-hidden group shadow-inner">
             ...
             <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-center z-10 w-full px-4">
                <h2 className="text-xl font-black italic uppercase text-white tracking-tighter drop-shadow-lg">{meta.circuit_name}</h2>
                <p className="text-[8px] text-white/40 uppercase tracking-[0.4em] font-bold">{meta.location}</p>
             </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-8 pt-4 border-t border-[#1F1F1F]">
        <div className="lg:col-span-7 bg-[#1F1F1F]/10 border border-[#1F1F1F] rounded-xl overflow-hidden shadow-2xl">
          <div className="bg-[#1F1F1F] px-4 py-2 text-[10px] font-bold uppercase tracking-widest text-[#FF1801] flex justify-between">
             ...
             <span className="text-white/20 font-mono">LIVE_STREAM</span>
          </div>
                   ...
                   {stints.map((s, i) => (
                     <tr key={i} className="border-t border-white/5 hover:bg-white/[0.02]">
                        ...
                        <td className="p-2 text-center">
                           <span className={`px-3 py-0.5 rounded-sm font-bold text-[8px] ${s.compound === 'SOFT' ? 'bg-red-600 text-white' : s.compound === 'MEDIUM' ? 'bg-yellow-600 text-white' : 'bg-white text-black'}`}>
                             {s.compound}
                           </span>
                        </td>
      ...
      <div className="border border-[#1F1F1F] bg-[#1F1F1F]/10 rounded-xl p-6 shadow-2xl mb-20">
        <div className="flex justify-between items-center mb-6 px-2">
           <div className="flex items-center gap-3">
              ...
              <div className="px-2 py-0.5 bg-green-500/20 text-green-500 text-[8px] font-bold rounded animate-pulse uppercase">Link_Active</div>
           </div>
           <span className="text-[8px] text-white/20 uppercase font-mono tracking-widest">Global Hz: 2.7</span>
        </div>
            ...
            return (
              <div key={num} className="bg-[#1F1F1F]/40 border border-[#1F1F1F] p-3 rounded-lg hover:border-[#FF1801]/50 transition-all group">
                <div className="flex justify-between items-center mb-3">
                  ...
                  <div className="w-1 h-3 rounded-full shadow-lg" style={{ backgroundColor: TEAM_COLORS[num] }}></div>
                </div>
