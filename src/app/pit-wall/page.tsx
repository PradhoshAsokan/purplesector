...
export default function PitWallPage() {
  ...
  if (!sessionActive) {
     return (
        <div className="p-8 bg-black font-mono text-white min-h-screen">
           <div className="max-w-6xl mx-auto">
              <header className="flex justify-between items-end mb-12 border-b border-[#1F1F1F] pb-8">
                 <div>
                    <h1 className="text-6xl font-black text-[#FF1801] uppercase italic tracking-tighter">Session Summary</h1>
                    <p className="text-xl text-white/40 uppercase tracking-widest">{sessionName} — {meta.circuit_name}</p>
                 </div>
                 <div className="text-right">
                    <p className="text-xs text-white/20 uppercase tracking-[0.3em] mb-1">Status</p>
                    <span className="px-4 py-1 bg-white/5 border border-white/10 rounded-full text-white font-bold text-sm uppercase">Off-Track</span>
                 </div>
              </header>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                 ...
                 <div className="lg:col-span-2 space-y-6">
                    <h3 className="text-[#FF1801] font-black italic uppercase tracking-widest text-sm underline decoration-2 underline-offset-8 mb-8">Final Classification</h3>
                    <div className="bg-[#1F1F1F]/20 border border-[#1F1F1F] rounded-2xl overflow-hidden shadow-2xl">
                       <table className="w-full text-left">
                          <thead className="bg-[#1F1F1F] text-white/40 text-[10px] uppercase font-bold tracking-widest">
                          ...
                          <tbody>
                             {results.map((r) => (
                                <tr key={r.driver_number} className="border-t border-white/5 hover:bg-white/[0.02] transition-colors">
                                   <td className="p-4 text-2xl font-black italic text-white/20">{r.position}</td>
                                   <td className="p-4">
                                      <div className="flex items-center gap-4">
                                         <div className="w-1 h-8 rounded-full shadow-glow" style={{ backgroundColor: TEAM_COLORS[r.driver_number] }}></div>
                                         <div>
                                            <p className="text-xl font-black uppercase italic leading-none text-white tracking-tighter">
                                               {DRIVER_CODES[r.driver_number] || 'DVR'}
                                            </p>
                                            <p className="text-[10px] text-white/40 font-bold mt-1 tracking-widest">
                                               #{r.driver_number}
                                            </p>
                                         </div>
                                      </div>
                                   ...
                                   <td className="p-4 text-xs font-bold uppercase">
                                      {r.dsq ? <span className="text-[#FF1801]">DSQ</span> : 
                                       r.dnf ? <span className="text-white/40">DNF</span> : 
                                       r.dns ? <span className="text-white/20">DNS</span> : 
                                       <span className="text-green-500/60 text-[8px]">Finished</span>}
                                   </td>
                                   <td className="p-4 text-right text-2xl font-black text-[#FF1801] tabular-nums">{r.points}</td>
                                </tr>
                             ))}
                          </tbody>
                       </table>
                 ...
                 <div className="space-y-8">
                    <div className="bg-[#1F1F1F]/20 border border-[#1F1F1F] rounded-2xl p-6 shadow-2xl relative overflow-hidden group">
                       <h4 className="text-[10px] font-black text-[#FF1801] uppercase tracking-widest mb-6">Track Specs</h4>
                    ...
                    <div className="bg-[#FF1801] rounded-2xl p-8 shadow-[0_0_50px_rgba(255,24,1,0.2)]">
                       ...
                       <div className="text-4xl font-black text-white tracking-tighter italic">LIVE IN 12 DAYS</div>
                    </div>
                 </div>
              </div>
           </div>
  ...
  return (
    <div className="p-4 bg-black font-mono text-white min-h-screen overflow-x-hidden">
      
      {/* LAYER 1: LIVE CONTROL */}
      <div className="h-[calc(100vh-120px)] flex flex-col mb-8">
        <div className="flex justify-between items-center mb-4 px-2">
          <div className="flex items-center gap-4">
            <h1 className="text-2xl font-black text-[#FF1801] uppercase italic tracking-tighter text-glow">The Pit Wall</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 flex-1 min-h-0">
          ...
          <div className="lg:col-span-3 h-full flex flex-col bg-[#1F1F1F]/20 border border-[#1F1F1F] rounded-lg overflow-hidden shadow-2xl">
            <div className="bg-[#1F1F1F] px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest text-[#FF1801]">Timing / Gaps</div>
                ...
                <tbody className="bg-black/20">
                  {leaderboard.map((d) => (
                    <tr key={d.driver_number} className="border-b border-white/[0.03] hover:bg-white/5 transition-colors h-7">
                      ...
                      <td className="px-2 py-1 text-[#FF1801] tabular-nums font-bold tracking-tighter text-right">{d.gap}</td>
                      <td className="px-2 py-1 text-white/30 tabular-nums text-[9px] text-right">{d.interval}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="lg:col-span-6 h-full border border-[#1F1F1F] bg-[#1F1F1F]/10 rounded-lg flex flex-col items-center justify-center relative overflow-hidden group shadow-inner">
             ...
             <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-center z-10 w-full px-4"><h2 className="text-xl font-black italic uppercase text-white tracking-tighter drop-shadow-lg">{meta.circuit_name}</h2><p className="text-[8px] text-white/40 uppercase tracking-[0.4em] font-bold">{meta.location}</p></div>
          </div>
          ...
          <div className="lg:col-span-3 h-full flex flex-col bg-[#1F1F1F]/20 border border-[#1F1F1F] rounded-lg overflow-hidden shadow-2xl">
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-2 text-[10px] scrollbar-thin scrollbar-thumb-white/10">
              {messages.map((m, i) => (
                <div key={i} className="border-l-2 border-[#FF1801] pl-3 py-1 bg-white/[0.03] rounded-r-md"><p className="text-white/90 leading-relaxed">{m.message}</p></div>
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-8 pt-4 border-t border-[#1F1F1F]">
        <div className="lg:col-span-7 bg-[#1F1F1F]/10 border border-[#1F1F1F] rounded-xl overflow-hidden shadow-2xl">
          <div className="bg-[#1F1F1F] px-4 py-2 text-[10px] font-bold uppercase tracking-widest text-[#FF1801] flex justify-between"><span>Tire Strategy & Pit Analysis</span><span className="text-white/20 font-mono uppercase text-[8px]">Link_Active</span></div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4">
             <div className="border border-white/5 rounded-lg overflow-hidden bg-black/40">
               <table className="w-full text-left text-[10px]">
                 <thead className="bg-white/5 text-white/40 uppercase font-bold"><tr><th className="p-2">Car</th><th className="p-2">Comp</th><th className="p-2">Age</th></tr></thead>
                 <tbody>{stints.map((s, i) => (<tr key={i} className="border-t border-white/5 hover:bg-white/[0.02]"><td className="p-2 font-black italic">#{s.driver_number}</td><td className="p-2 text-center"><span className={`px-3 py-0.5 rounded-sm font-bold text-[8px] ${s.compound === 'SOFT' ? 'bg-red-600 text-white' : s.compound === 'MEDIUM' ? 'bg-yellow-600 text-white' : 'bg-white text-black'}`}>{s.compound}</span></td><td className="p-2 text-white/60 tabular-nums">{s.tyre_age} L</td></tr>))}</tbody>
               </table>
             </div>
             <div className="border border-white/5 rounded-lg overflow-hidden flex flex-col bg-black/40">
                <div className="bg-white/5 p-2 text-[8px] font-bold uppercase text-white/40 border-b border-white/5 text-center">Box Entry Timeline</div>
                <div className="flex-grow p-2 space-y-2 max-h-[160px] overflow-y-auto">{pits.map((p, i) => (<div key={i} className="flex justify-between items-center text-[10px] p-2 bg-white/5 border border-white/5 rounded-md group hover:border-[#FF1801]/40 transition-all"><div className="flex items-center gap-2"><div className="w-1.5 h-1.5 bg-[#FF1801] rounded-full"></div><span className="font-black italic text-xs">#{p.driver_number}</span></div><div className="flex gap-4 items-center"><span className="text-[#FF1801] font-bold tabular-nums">{p.duration ? p.duration.toFixed(2) : '---'}s</span><span className="text-white/20 text-[8px] font-bold uppercase">L{p.lap_number || '---'}</span></div></div>))}</div>
             </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 bg-[#1F1F1F]/10 border border-[#1F1F1F] rounded-xl overflow-hidden shadow-2xl">
          ...
          <div className="p-4 space-y-3 h-[300px] overflow-y-auto custom-scrollbar">{radio.map((r, i) => (<div key={i} className="p-3 bg-white/[0.02] border border-white/5 rounded-lg hover:border-[#FF1801]/40 transition-all group"><div className="flex items-center justify-between mb-2"><div className="flex items-center gap-2"><div className="w-6 h-6 rounded bg-[#FF1801] flex items-center justify-center text-[10px] font-black italic shadow-lg shadow-[#FF1801]/20">{r.driver_number}</div><span className="text-[9px] font-bold text-[#FF1801] uppercase tracking-tighter">{DRIVER_CODES[r.driver_number] || 'DVR'} UPLINK</span></div><button onClick={() => playRadio(r.recording_url)} className="text-[8px] font-black uppercase tracking-widest px-3 py-1 bg-[#FF1801] text-white rounded shadow-xl hover:scale-105 transition-transform">Listen</button></div><div className="flex items-center gap-4"><div className="h-[2px] flex-1 bg-white/5 relative overflow-hidden"><div className="absolute inset-0 bg-[#FF1801]/20 animate-[loading_2s_ease-in-out_infinite]"></div></div><span className="text-[8px] text-white/20 uppercase tabular-nums">{new Date(r.date).toLocaleTimeString()}</span></div></div>))}</div>
        </div>
      </div>

      {/* LAYER 3: TELEMETRY */}
      <div className="border border-[#1F1F1F] bg-[#1F1F1F]/10 rounded-xl p-6 shadow-2xl mb-20">
        <div className="flex justify-between items-center mb-6 px-2"><div className="flex items-center gap-3"><h3 className="text-[#FF1801] font-black italic uppercase text-sm tracking-widest">Telemetry grid</h3><div className="px-2 py-0.5 bg-green-500/20 text-green-500 text-[8px] font-bold rounded animate-pulse uppercase">Link_Active</div></div><span className="text-[8px] text-white/20 uppercase font-mono tracking-widest">Global Hz: 2.7</span></div>
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">{Object.keys(DRIVER_CODES).map((num) => { const dNum = parseInt(num); const data = telemetry[dNum]; return (<div key={num} className="bg-[#1F1F1F]/40 border border-[#1F1F1F] p-3 rounded-lg hover:border-[#FF1801]/50 transition-all group"><div className="flex justify-between items-center mb-3"><span className="text-base font-black italic text-white/40 group-hover:text-white transition-colors">{DRIVER_CODES[num]}</span><div className="w-1 h-3 rounded-full shadow-lg" style={{ backgroundColor: TEAM_COLORS[num] }}></div></div><div className="space-y-2"><TelemetryStat label="SPD" value={data?.speed.toString() || '---'} unit="KM/H" /><TelemetryStat label="RPM" value={data?.rpm.toString() || '----'} /><TelemetryStat label="GR" value={data?.n_gear.toString() || '-'} /><div className="h-1 w-full bg-white/5 rounded-full mt-2 overflow-hidden shadow-inner"><div className="h-full bg-[#FF1801]/60 transition-all duration-700" style={{ width: `${(data?.throttle || 0)}%` }}></div></div></div></div>); })}</div>
      </div>
    </div>
  );
}
...
function CircuitStat({ label, value }: { label: string, value: string }) {
  return (
    <div className="bg-black/60 p-2 rounded border border-white/5 backdrop-blur-md min-w-[85px] shadow-lg">
